            ...
            while(start)
            {
                ...
                else if (key.Key == ConsoleKey.Enter)
                {
                    ...
                    else
                    {
                        ...
                        {
                            if (File.Exists("E:\\Order.txt"))
                            {
                                File.AppendAllText("E:\\Order.txt", "\n");
                                File.AppendAllText("E:\\Order.txt", DateTime.Now + "\n" + "Общая цена: " + fin_Price + "\n" + "Заказ: " + "\n");
                                foreach (var element in alr_torted)
                                {
                                    File.AppendAllText("E:\\Order.txt", element.Name + " - " + element.Price + "\n");
                                }
                            }
                            else
                            {
                                File.WriteAllText("E:\\Order.txt", DateTime.Now + "\n" + "Общая цена: " + fin_Price + "\n" + "Заказ: " + "\n");
                                foreach (var element in alr_torted)
                                {
                                    File.AppendAllText("E:\\Order.txt", element.Name + " - " + element.Price + "\n");
                                }
